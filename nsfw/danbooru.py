...
class DanbooruClient:
	...
	async def get_images(self, tag):
		...
		if await self.cache.get(tag) is None:
			async with aiohttp.ClientSession() as session:
				async with session.get("https://danbooru.donmai.us/posts.json", params={"tags": tag, "limit": 100}, headers=headers) as response:
	...
	async def get_latest_posts(self):
		images = dict()
		async with aiohttp.ClientSession() as session:
			async with session.get(home_page + "/posts?tags=rating%3Aexplicit+&ms=1", headers=headers) as response:
				if response.status == 200:
						...
						if post.get("data-id", None) is not None:
							...
							async with session.get(source, headers=headers) as response:
								if response.status == 200:
									page_html = await response.text()
									soup = BeautifulSoup(page_html, 'lxml')
									title = soup.find('title').string.replace(" | Danbooru", "").replace("'", "").replace("\"", "")
									images[title] = {
										"name": title,
										"type": "image",
										"product": img,
	...
	async def update_catalog_task(self):
		while True:
			...
			if not "danbooru" in list(await core.utils.shop_catalog()):
				await core.utils.shop_catalog(catalog="danbooru", nsfw=True, content=content, mode="c")
			else:
				await core.utils.shop_catalog(catalog="danbooru", content=content, mode="w")
