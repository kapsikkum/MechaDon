	...
	async def get_images(self, tag):
		...
		if await self.cache.get(tag) is None:
			async with aiohttp.ClientSession() as session:
				async with session.get("https://hypnohub.net/post/index.json", params={"tags": tag, "limit": 100}, headers=headers) as response:
								...
								try:
									if post['file_url'].split(".")[-1] in ['mp4', 'webm']:
										...
										images.append({
											...
											"post_url": f"https://hypnohub.net/post/show/{post['id']}"
										})
								except:
									pass
		else:
