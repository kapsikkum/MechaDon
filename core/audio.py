            ...
            if not isinstance(source, YTDLSource):
                ...
                try:
                    ...
                    await self._channel.send(
                        embed=discord.Embed(
                            title=f"There was an error processing your song.",
                            description=str(e),
                            color=0xFF0000,
                        )
                    )
...
class Audio_Commands(commands.Cog):
    ...
    @commands.command(
        name="connect", aliases=["join"], description="Connect to a voice channel."
    )
    @commands.guild_only()
    ...
    @commands.command(
        ...
        usage="{prefix}volume <float between 1 and 100>",
    )
    @commands.guild_only()
    @commands.cooldown(1, 5, commands.BucketType.user)
