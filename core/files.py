# -*- coding: utf-8 -*-
# @Author: Blakeando
# @Date:   2020-08-13 14:22:59
# @Last Modified by:   Blakeando
# @Last Modified time: 2020-08-13 14:22:59
import asyncio
import io
import tempfile
import uuid
...
class File_Commands(commands.Cog):
    ...
    @commands.command(
        description="Checks MD5 of an uploaded file.",
        usage="{prefix}md5 <attachment>",
        name="md5",
        aliases=["checkmd5"],
    )
    ...
    async def _md5(self, ctx):
        if len(ctx.message.attachments) == 0:
            ...
            message = await ctx.send(
                embed=discord.Embed(
                    title="Please Wait", description="Checking File...", color=0xFF0000
                )
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(attachment) as r:
                    if r.status == 200:
                        ...
                        await message.edit(
                            embed=discord.Embed(
                                title=f"MD5 of {attachment.split('/')[-1]}:",
                                description=md5,
                                color=0xFF0000,
                            )
                        )
    ...
    @commands.command(
        description="Create a solid colour image from a hex value.",
        usage="{prefix}colour <hex value>",
        aliases=["color"],
    )
    ...
    @commands.command(
        description="Create a random colour image.", aliases=["randomcolor", "rdmc"]
    )
    @commands.cooldown(1, 10, commands.BucketType.user)
    async def randomcolour(self, ctx):
        colour = "#%02x%02x%02x" % (
            core.utils.random.randint(0, 255),
            core.utils.random.randint(0, 255),
            core.utils.random.randint(0, 255),
        )
        with tempfile.TemporaryDirectory() as tempdir:
    ...
    @commands.command(
        description="Create a Don",
        usage="{prefix}don <optional hex colour>",
        aliases=["randon"],
    )
    ...
    async def don(self, ctx, colour="random"):
        ...
        with ctx.typing():
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                embed = discord.Embed(
                    title="Da-don",
                    description=f"HEX: `{'%02x%02x%02x' % (colour[0], colour[1], colour[2])}`",
                    color=0xFF0000,
                )
                embed.set_image(url=f"attachment://{filename.split('/')[-1]}")
                await ctx.send(file=file, embed=embed)
    ...
    @commands.command(
        description="Create a .zip file full of random Dons.",
        usage="{prefix}massdon <optional number of dons>",
        aliases=["zipdon"],
    )
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                with ZipFile(zipname, "w") as zip_file:
                    if amount > 175:
                        ...
                        await ctx.send(
                            embed=discord.Embed(
                                title="Way too many dons!",
                                description=f"Max is {amount}, so i'm only making that many.",
                                color=0xFF0000,
                            )
                        )
    ...
    @commands.command(
        description="Creates Text To Speech .mp3 file (From Google)",
        ...
        async with ctx.typing():
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    "https://www.google.com/speech-api/v1/synthesize", params=params
                ) as r:
                    if r.status == 200:
                        with tempfile.TemporaryDirectory() as tempdir:
                            async with aiofiles.open(
                                f"{tempdir}/{ctx.message.author.id}.mp3", mode="wb"
                            ) as f:
                                await f.write(await r.read())
                            await ctx.send(
                                file=discord.File(
                                    f"{tempdir}/{ctx.message.author.id}.mp3",
                                    filename=f"{ctx.message.author.id}.mp3",
                                )
                            )
                    else:
    ...
    async def bonzi(self, ctx, *, args):
        ...
        async with ctx.typing():
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    "https://tetyys.com/SAPI4/SAPI4", params=params
                ) as r:
                    if r.status == 200:
                        with tempfile.TemporaryDirectory() as tempdir:
                            sound = await self.bot.loop.run_in_executor(
                                ThreadPoolExecutor(),
    ...
    @commands.command(
        description="DUKI NUKI", usage="{prefix}duki <text>", aliases=["dukinuki"]
    )
    @commands.cooldown(1, 10, commands.BucketType.user)
