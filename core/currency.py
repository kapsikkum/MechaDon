# -*- coding: utf-8 -*-
# @Author: Blakeando
# @Date:   2020-08-13 14:23:46
# @Last Modified by:   Blakeando
# @Last Modified time: 2020-08-13 14:23:46
import asyncio
import json
...
class Currency_Commands(commands.Cog):
    ...
    @commands.command(
        aliases=["bal"],
        description="Check the balance of you or another person.",
        ...
        if user is None:
            ...
            await ctx.send(
                embed=discord.Embed(
                    ...
                    description=f"${(0 if cash is None else cash)}",
                    color=0xFF0000,
                )
            )
        else:
    ...
    async def collect(self, ctx):
        ...
        if cash is None:
            cash = await core.utils.add_user(ctx.message.author.id)
        cash = await core.utils.add_cash(
            ctx.message.author.id, core.user_currency[str(ctx.message.author.id)]
        )
        await ctx.send(
            embed=discord.Embed(
                title=f"Collected ${round(core.user_currency[str(ctx.message.author.id)], 2)}",
                description=f"Current Balance: ${cash}",
                color=0xFF0000,
            )
        )
                ...
                if mode == "buy":
                    if item is not None:
                        if store is not None:
                            if store in stores:
                                try:
                                    item = list(stores[store]["catalog"].keys())[
                                        int(item) - 1
                                    ]
                                except:
                                    raise core.exceptions.CommandError(
                                        "Not a Valid Shop item."
                                    )
                                else:
                                    purchase, catalog = await core.utils.purchase_item(
                                    ...
                                    if purchase is None:
                                        await ctx.send(
                                            embed=discord.Embed(
                                                title=f"You don't have the funds for this.",
                                                description="Get some cash loser poor boy",
                                                color=0xFF0000,
                                            )
                                        )
                                    else:
                                        ...
                                        await core.utils.shop_catalog(
                                            catalog=store,
                                            content=stores[store]["catalog"],
                                            mode="w",
                                        )
                                        await ctx.send(
                                            embed=discord.Embed(
                                                title=f"Item Purchased.",
                                                description="Sending you the item in DMs now.",
                                                color=0xFF0000,
                                            )
                                        )
                                        embed = discord.Embed(
                ...
                if store is not None:
                    if store in stores:
                        ...
                        for item in list(stores[store]["catalog"]):
                            counter += 1
                            if (
                                not ctx.channel.is_nsfw()
                                and stores[store]["catalog"][item]["nsfw"]
                            ):
                                ...
                                store_page.add_field(
                                    name=f"**{counter}:**",
                                    value=f" ***__{item}__*** \nPrice: **${round(stores[store]['catalog'][item]['price'], 2)}**\n Quantity: {stores[store]['catalog'][item]['quantity'] if not stores[store]['catalog'][item]['permanent'] else '∞'}",
                                    inline=False,
                                )
                        if stores[store]["nsfw"]:
                            if isinstance(ctx.channel, discord.DMChannel):
                                ...
                                await ctx.send(
                                    embed=discord.Embed(
                                        ...
                                        description="Please do this command in a NSFW channel or DMs.",
                                        color=0xFF0000,
                                    )
                                )
                        else:
        ...
        if purchase is None:
            ...
            if core.utils.random.randint(1, 10) == 1:
                ...
                await ctx.send(
                    embed=discord.Embed(
                        title="You Won!",
                        description=f"Winnings: ${round(winnings, 2)}",
                        color=0xFF0000,
                    )
                )
            else:
    ...
    @commands.command(
        description="Check the items you have bought.",
        ...
        if len(items) == 0:
            await ctx.send(
                embed=discord.Embed(
                    ...
                    description="Buy some from the shop.",
                    color=0xFF0000,
                )
            )
        else:
            if view is None:
                items_embed = discord.Embed(
                    title=f"Your items", description=discord.Embed.Empty, color=0xFF0000
                )
                counter = 0
                for item in items:
                    counter += 1
                ...
                try:
                    ...
                    await ctx.send(
                        embed=discord.Embed(
                            title=f"Invalid Item.",
                            description="Select the right one next time.",
                            color=0xFF0000,
                        )
                    )
                else:
                    await ctx.send(
                        embed=discord.Embed(
                            title=f"OK.",
                            description="Sending you your item in DMs now.",
                            color=0xFF0000,
                        )
                    )
                    embed = discord.Embed(
                        title=f"{items[item]['name']}",
                        description="This is your item.",
                        color=0xFF0000,
                    )
                    if items[item]["type"] == "image":
                        embed.set_image(url=items[item]["product"])
        ...
        if not user:
            await ctx.send(
                embed=discord.Embed(
                    ...
                    description="Can't give cash to nobody can I?",
                    color=0xFF0000,
                )
            )
        else:
            ...
            await ctx.send(
                embed=discord.Embed(
                    title=f"Gave ${amount} to {user.name}.",
                    description="Ruining the economy I see.",
                    color=0xFF0000,
                )
            )
    ...
    async def setcash(self, ctx, user: discord.User = None, amount=0):
        if not user:
            ...
            await ctx.send(
                embed=discord.Embed(
                    title=f"Set {user.name}'s balance to ${amount}.",
                    description='"i am crippeld in debt rn bru"\n- Mattlau04',
                    color=0xFF0000,
                )
            )
        ...
        if ctx.invoked_subcommand is None:
            await ctx.send(
                embed=discord.Embed(
                    ...
                    description="Dumbass can't use his own bot LMAO",
                    color=0xFF0000,
                )
            )
        ...
        if name is not None:
            if len(ctx.message.attachments) == 0:
                if data is not None:
                    ...
                    await core.utils.shop_catalog(
                        catalog="general", content=item, name=name, mode="a"
                    )
            else:
