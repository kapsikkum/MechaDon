# -*- coding: utf-8 -*-
# @Author: Blakeando
# @Date:   2020-08-13 14:23:10
# @Last Modified by:   Blakeando
# @Last Modified time: 2020-08-13 14:23:10
import asyncio
import io
import re
import time
...
class Basic_Commands(commands.Cog):
    ...
    async def help(self, ctx, cmd=None):
        if cmd is None:
            ...
            for category in sorted(categories):
                ...
                if ctx.message.guild is not None:
                    embed.add_field(
                        name=f"**__{category.replace('_', ' ')}__**",
                        value="\n".join(
                            [
                                f"**{core.utils.prefix_for(ctx.message.guild)}{command}**"
            ...
            if com is None:
                ...
                if com.hidden:
                    ...
                    if ctx.message.guild is not None:
                        embed = discord.Embed(
                            title=f"Command info for {core.utils.prefix_for(ctx.message.guild)}{com.name}",
                            description=discord.Embed.Empty,
                            color=0xFF0000,
                        )
                    else:
                        embed = discord.Embed(
                            title=f"Command info for {core.prefix}{com.name}",
                            description=discord.Embed.Empty,
                            color=0xFF0000,
                        )
                    if com.description == "":
                        ...
                        embed.add_field(
                            name="**Description:**", value=com.description, inline=False
                        )
                    if len(com.aliases) > 0:
                        embed.add_field(
                            name="**Aliases:**",
                            value=", ".join(sorted(com.aliases)),
                            inline=False,
                        )
                    if com.usage is None:
                        if ctx.message.guild is not None:
                            embed.add_field(
                                name="**Usage:**",
                                value=f"{core.utils.prefix_for(ctx.message.guild)}{com.name}",
                                inline=False,
                            )
                        else:
                            embed.add_field(
                                name="**Usage:**",
                                value=f"{core.prefix}{com.name}",
                                inline=False,
                            )
                    else:
                        if ctx.message.guild is not None:
                            embed.add_field(
                                name="**Usage:**",
                                value=str(com.usage).format(
                                    prefix=core.utils.prefix_for(ctx.message.guild)
                                ),
                                inline=False,
                            )
                        else:
                            embed.add_field(
                                name="**Usage:**",
                                value=str(com.usage).format(prefix=core.prefix),
                                inline=False,
                            )
                    await ctx.send(embed=embed)
    ...
    async def whois(self, ctx, user=None):
        ...
        if user:
            ...
            try:
                ...
                embed = discord.Embed(
                    title=f"Info on {user.name}", description="", color=0xFF0000
                )
                ...
                embed.add_field(
                    name="**Timestamps**",
                    value=f"**Account Created**: {user.created_at.strftime('%d/%m/%Y %I:%M %p')} ({humanize.naturaltime(user.created_at)})",
                    inline=False,
                )
            else:
                user = user_member
                ...
                embed.add_field(
                    name="**Timestamps**",
                    value=f"**Date Joined**: {user.joined_at.strftime('%d/%m/%Y %I:%M %p')} ({humanize.naturaltime(datetime.now().replace(tzinfo=tz.tzlocal()) - joined_at.astimezone(tz.tzlocal()))})\n**Account Created**: {user.created_at.strftime('%d/%m/%Y %I:%M %p')} ({humanize.naturaltime(datetime.now().replace(tzinfo=tz.tzlocal()) - created_at.astimezone(tz.tzlocal()))})",
                    inline=False,
                )
            await ctx.send(embed=embed)
        else:
    ...
    @commands.command(
        description="Gets info on a specified invite.",
        usage="{prefix}inviteinfo https://discord.gg/cumzone | discord.gg/cumzone | cumzone",
        aliases=["invinf"],
    )
    ...
    async def inviteinfo(self, ctx, inv: discord.Invite = None):
        if inv is not None:
            ...
            embed = discord.Embed(
                title=f"Info on invite to {inv.guild.name}",
                description=discord.Embed.Empty,
                color=0xFF0000,
            )
            ...
            embed.add_field(
                name="**Server Info**",
                value=f"**Name**: {inv.guild.name}\n**ID**: {inv.guild.id}\n**Members**: {inv.approximate_member_count} ({inv.approximate_presence_count} online)",
                inline=False,
            )
            embed.add_field(
                name="**Invite Info**",
                value=f"**URL**: {inv.url}\n**Code**: {inv.id}\n**Channel Name**: {inv.channel.name}\n**Channel ID**: {inv.channel.id}\n**Channel Type**: {inv.channel.type}",
                inline=False,
            )
    ...
    @commands.command(
        description="Make the bot say something.", usage="{prefix}say <message>"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def say(self, ctx, *args):
        await ctx.send(
            " ".join(args).replace("@everyone", "@nobody").replace("@here", "@there")
        )

    @commands.command(
        description="Make a random string of words.", aliases=["rw", "ranword"]
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def randomwords(self, ctx, length: int = 10):
        ...
        for partition in [words[i : i + 2000] for i in range(0, len(words), 2000)]:
            await ctx.send(
                embed=discord.Embed(
                    title=discord.Embed.Empty, description=partition, color=0xFF0000
                )
            )
    ...
    @commands.command(
        ...
        usage="{prefix}saynumber <number>",
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def saynumber(self, ctx, number):
        await ctx.send(
            embed=discord.Embed(
                title=f"{number} in words:",
                description=core.utils.number_to_word(number),
                color=0xFF0000,
            )
        )
    ...
    async def vore(self, ctx, user: discord.Member = None):
        if ctx.message.author.id == user.id:
            embed = discord.Embed(
                title="Oh...",
                description=r"Ť̫̣̬͓̹͙͚̹̖̬̹̬̹̟̙̼̥͖ͭ̇͐̅ͨ̋ͮ̌͆̍ͨ̓ͥ͢͠H̨̧͚̳̖̲͓̫̘̲͉͍̼̞͋̂͌̾̕͟E̡ͦ́̀ͨͬ̋ͪ̈́͆̍͂̋͊ͭ̐̕͠҉̯̬̗͔̜͍͚̟͙ ͈̳̺̳̖̩̯͙̜̔ͪ́ͮ̿͟͟V̶̸̪̬͕̗̙̳͕̊ͨͫ͛̈ͭ͊ͬ̏ͨͦͥ̀̚͜͜Ōͭ͌͊͊̃͑̈́̓͌́̐̊̾̿̇̓̒͏̖̯͇̰̪̹̱̕I̸̍ͬ̇̎̈́̃̎̉̃͑̀͆̅́̏̋͂ͨ̚͘͝҉̥̞̖̩̹̬̹̭̤͎D̵̢̬̲̩͈̈́ͪͨ̋͐͒͟͞͞ ̸̄̄ͬ͑͗́ͩ́͝͏̠͕̯͍̪̮͠ͅĜ̸̰̖̳͇͓̰̹̹̩̟̣̖͎̦̙ͪͦ͌̃̆͒͟͜͞R̡̭̰̪͈̻̪͇̠͑ͯ̈̈́̎͆ͩ̉̄̀̀̋̋̅̅ͧ̎͂̚͘͘͟͟Ȍ̷̡̫͈̪̞̯̃̎̊̓͢Wͨ͂͋ͥ̽̈́̈͗͒ͬͬ̈ͧ̉̏̇͊҉҉̯̩͕̭̣͎̮̫̀S̡̛͍̮̹̲͇̘̘͎̬̱̻͈̼̀̽̇̄͌͑̎ͧ̾ͩ͐̅͂̉̐̓̀͡͝ͅ",
                color=0xFF0000,
            )
            embed.set_image(
                url="https://cdn.discordapp.com/attachments/686523390806327314/700731647988990042/8FuH.png"
            )
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(
                title="Vore",
                description=f"{ctx.message.author.name} has vored {user.name}!",
                color=0xFF0000,
            )
            embed.set_image(
                url="https://cdn.discordapp.com/attachments/686523390806327314/698912529493000212/fastvore.gif"
            )
            await ctx.send(embed=embed)
    ...
    async def kill(self, ctx, user: discord.Member = None):
        if ctx.message.author.id == user.id:
            embed = discord.Embed(
                title="Suicide",
                description=f"{ctx.message.author.name} killed themselves LMAO",
                color=0xFF0000,
            )
            embed.set_image(
                url="https://cdn.discordapp.com/attachments/686523390806327314/700730485407481906/killself.gif"
            )
            await ctx.send(embed=embed)
        else:
            actions = [
            ...
            images = [
                "https://cdn.discordapp.com/attachments/686523390806327314/698491736892112906/tenor_2.gif",
            ...
            embed = discord.Embed(
                title="Murder",
                description=f"**{ctx.message.author.name}** {core.utils.random.choice(actions)} **{user.name}** {core.utils.random.choice(results)} {core.utils.random.choice(locations)} {core.utils.random.choice(persons)}. {core.utils.random.choice(sayings)}",
                color=0xFF0000,
            )
    ...
    async def botinfo(self, ctx):
        embed = discord.Embed(
            title="Bot Info",
            description=f"Member of **{len(self.bot.guilds)}** Guild(s)",
            color=0xFF0000,
        )
        embed.add_field(name="**Bot Version**", value=core.__version__, inline=False)
        embed.add_field(
            name="**Uptime**",
            value=timedelta(seconds=int(time.time()) - int(core.uptime)),
            inline=False,
        )
        try:
            embed.add_field(
                name="**Prefix for guild**",
                value=core.utils.prefix_for(ctx.message.guild),
                inline=False,
            )
        except:
            pass
    ...
    async def domino(self, ctx):
        await ctx.send(
            "https://cdn.discordapp.com/attachments/680748917511421959/710129074294751252/splash.mp4"
        )
